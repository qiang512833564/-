...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        //缓冲更新队列
        self.updateListSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_DATA_ADD, 0, 0, dispatch_get_main_queue());//创建dispatch源，这里使用加法来合并dispatch源数据，最后一个参数是指定dispatch队列
        dispatch_source_set_event_handler(self.updateListSource, ^{//设置响应dispatch源事件的block，在dispatch源指定的队列上运行
            //可以通过dispatch_source_get_data(source)来得到dispatch源数据
            [self conversationListUpdate];
            
        });
        dispatch_resume(self.updateListSource);//dispatch源创建后处于suspend状态，所以需要启动dispatch源
...
#pragma mark - EMChatManagerUtilDelegate 方法 监听网络变化，尝试重新登录  
...
#pragma mark - 检测会话是否已经存在 --- 一般用于当加入某个群组的时候，检测改群组是否存在，再进行后面的操作（chatter -- 会话对方的用户名. 如果是群聊, 则是群组的id）
