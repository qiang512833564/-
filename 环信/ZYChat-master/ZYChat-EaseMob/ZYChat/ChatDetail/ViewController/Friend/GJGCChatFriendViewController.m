...
#pragma mark - 资源下载 （gif、image、视频截图）
- (void)downloadImageFile:(GJGCChatContentBaseModel *)contentModel forIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (imageContentModel.contentType == GJGCChatFriendContentTypeImage) {
        ...
        if (!imageMessageBody.thumbnailDownloadStatus) {// 其中 EMImageMessageBody 内部会加载内部文件内容，只是速度较慢，具体原因是什么，不是太清楚(可以通过对imageMessageBody添加thumbnailDownloadStatus属性观察，得到验证)，所以到达这一步的时候，基本上已经处于EMAttachmentDownloading状态，
            // imageMessageBody.thumbnailDownloadStatus > EMAttachmentDownloadSuccessed
            GJCFWeakSelf weakSelf = self;
            //[imageMessageBody addObserver:self forKeyPath:@"thumbnailDownloadStatus" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
            [[EaseMob sharedInstance].chatManager asyncFetchMessage:imageMessageBody.message progress:self completion:^(EMMessage *aMessage, EMError *error) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"thumbnailDownloadStatus"]) {
        NSUInteger state = [[change objectForKey:NSKeyValueChangeNewKey] integerValue];
        EMImageMessageBody *imageMessageBody = (EMImageMessageBody *)object;
        BOOL isSuccess = NO;
        switch (state) {
            case EMAttachmentDownloading:
                
                break;
            case EMAttachmentDownloadSuccessed:
            {
                isSuccess = true;
            }
                break;
            case EMAttachmentDownloadFailure:
                
                break;
            case EMAttachmentNotStarted:
                
                break;
            default:
                break;
        }
         [self downloadFileCompletionForMessage:imageMessageBody.message successState:isSuccess];
    }
}
/// 目测这个简单的 app 中，只有大文件，才需要自己去去下载，比如 gif 文件等
